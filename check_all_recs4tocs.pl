...
my $bibBase = 'xxx01';
my $obDomainFile='/exlibris/aleph/u23_1/alephe/www_f_cze/obalkyknih-url';
my $dir4TOCs='/exlibris/aleph/u23_1/alephe/sctatch';
...
foreach (@recentlyChanged) {
   ...
   if ( scalar(@isbns) + scalar(@cnbs) + scalar(@oclcs) == 0 ) {
      ...
      next; }
   #ask obalkyknih.cz
   my $obURL="http://katalog.osu.cz/obalky/$obDomain/api/books?multi=[{";
   $obURL =~ s/[\n\r]//g;   $obURL =~ s/[\r\n]//g;
